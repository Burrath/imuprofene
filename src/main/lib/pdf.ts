...
export async function pdfToRawTextData(arrayBuffer: ArrayBuffer): Promise<pdfToRawTextDataRes[]> {
  const pdfParser = new PDFParser()

  const data = await new Promise<PDF2JSONOutput>((resolve, reject) => {
    pdfParser.on('pdfParser_dataReady', (pdfData) => resolve(pdfData as PDF2JSONOutput))
