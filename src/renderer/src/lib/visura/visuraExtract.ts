...
function getVisuraNumberFromRawDataV1(rawData: pdfToRawTextDataRes[]): string {
  ...
  const nVisuraSlot = rawData.find((e) => e.text.toLowerCase().includes('visura n'))
...
function getVisuraNumberFromRawDataV2(rawData: pdfToRawTextDataRes[]): string {
  const nVisuraTitleSlot = rawData.find((e) => e.text.toLowerCase().includes('numero pratica'))
...
function getComuneAndCodiceFromRawDataV1(rawData: pdfToRawTextDataRes[]): {
  ...
  const textRaw = rawData.find((e) => e.text.toLowerCase().includes('comune di'))
...
function getComuneAndCodiceFromRawDataV2(rawData: pdfToRawTextDataRes[]): {
  ...
  const titleSlot = rawData.find((e) => e.text.toLowerCase().includes('comune di'))
...
function getSituazioniFromRawDataV1(rawData: pdfToRawTextDataRes[]): iSituazioneVisura[] {
  ...
  rawData.forEach((record) => {
    if (isSituazioniRecordTitle(record.text.toLowerCase())) situazioniTitleRecord.push(record)
  })
  ...
  situazioniTitleRecord.forEach((titleRecord, index) => {
    const getRelevanSituazioneRecords = () => {
      ...
      const relevantRecordFromSituazione: pdfToRawTextDataRes[] = rawData.filter(
        (record) => record.y >= startRow && record.y <= endRow
      )
    ...
    const getDataFromRecordColumn = (
      ...
      if (colRecord) {
        const foglioColData = relevantRecordFromSituazione.filter((record) => {
          const posCondition =
            record.y > colRecord.y && isWithinDelta(record.avgX, colRecord.avgX, offset)
    ...
    const getSituazioneDate = (
      ...
      const noteRecord = getDataFromRecordColumn('derivanti', relevantRecordFromSituazione, 100)
    ...
    const getRendita = (relevantRecordFromSituazione: pdfToRawTextDataRes[]) => {
      ...
      if (renditaRecord) {
        const rendita = parseFloat(
          renditaRecord.text.toLowerCase().replace('euro ', '').replace('.', '').replace(',', '.')
        )
    ...
    const getUnità = (
      ...
      const unità: iUnitàVisura[] = rowUnitàUnique.map((rowUnità) => {
        ...
        const particella = particellaColRecord.find((e) => e.y === rowUnità)?.text
    ...
    const getSituazioneType = (relevantRecordFromSituazione: pdfToRawTextDataRes[]) => {
    ...
    const getImmobileType = (
      ...
      for (let i = 0; i < relevantRecordFromSituazione.length; i++) {
        ...
        if (record.text.toLowerCase().includes('dominicale')) return IMMOBILE_TYPE.TerrenoAgricolo
      }
...
function getSituazioniFromRawDataV2(rawData: pdfToRawTextDataRes[]): iSituazioneVisura[] {
  const situazioni: iSituazioneVisura[] = []
  ...
  const getRelevantSlots = (rawData: pdfToRawTextDataRes[]) => {
    ...
    const secondIndex =
      secondIndexRelative !== -1 ? recordIndex + 1 + secondIndexRelative : rawData.length
  ...
  situazioniSlots.forEach((sRecords) => {
    ...
    const getUnità = (sRecords: pdfToRawTextDataRes[]) => {
      ...
      linesY.forEach((y) => {
        ...
        const foglioIndex = lineRecords.findIndex((e) => e.text.toLowerCase().trim() === 'foglio')
        const foglio = foglioIndex !== -1 ? lineRecords[foglioIndex + 1] : undefined

        const particellaIndex = lineRecords.findIndex(
          (e) => e.text.toLowerCase().trim() === 'particella'
        )
        const particella = particellaIndex !== -1 ? lineRecords[particellaIndex + 1] : undefined

        const subalternoIndex = lineRecords.findIndex(
          (e) => e.text.toLowerCase().trim() === 'subalterno'
        )
        const subalterno = subalternoIndex !== -1 ? lineRecords[subalternoIndex + 1] : undefined
    ...
    const getCategoria = (sRecords: pdfToRawTextDataRes[]) => {
      const catLineIndex = sRecords.findIndex((e) => e.text.toLowerCase().trim() === 'categoria')
      if (catLineIndex !== -1) {
    ...
    const getRendita = (sRecords: pdfToRawTextDataRes[]) => {
      const renditaLineIndex = sRecords.findIndex((e) => e.text.toLowerCase().trim() === 'rendita:')
    ...
    const getSituazioneType = (sRecords: pdfToRawTextDataRes[]): SITUAZIONE_TYPE => {
      const annotazioniColX = sRecords.find((e) =>
        e.text.toLowerCase().includes('annotazioni:')
      )?.startX

      if (annotazioniColX) {
        const relevantRecords = sRecords.filter((e) => e.startX >= annotazioniColX)
        const text = relevantRecords
          .map((r) => r.text)
          .join(' ')
          .toLowerCase()

        if (text.includes('rendita non rettific')) return SITUAZIONE_TYPE.RenditaNonRettificata

        if (text.includes('rendita rettific')) return SITUAZIONE_TYPE.RenditaRettificata

        if (text.includes('rendita propost')) return SITUAZIONE_TYPE.RenditaProposta
      }

      return SITUAZIONE_TYPE.RenditaValidata
    }

    const getImmobileType = (sRecords: pdfToRawTextDataRes[]): IMMOBILE_TYPE => {
